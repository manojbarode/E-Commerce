...
import "./Cart.css";
import { addWishlist, deleteCartItem, fetchCartdata } from "../../api/cartApi";
import { useDispatch, useSelector } from "react-redux";
import { removeItem, updateQuantity, setCartItems } from "../../Redux/cartSlice";
...
const Cart = () => {
  ...
  const fetchCart = async () => {if (!isLoggedIn) {
      toast.warning("Login required to access wishlist");
      navigate("/login");
      return;
    }
  ...
  const handleQuantity = (productUid, type) => {
    const item = cartItems.find((i) => i.productUid === productUid);
    if (!item) return;
  ...
  const handleRemove = async (productUid) => {
    ...
    try {
      await deleteCartItem(productUid);
  ...
  const toggleWishlist =async (productUid) => {
    ...
    try {
        const res = await addWishlist(productUid);
  ...
  if (loading) return (
    <div className="cart-wrapper text-center mt-5">
          ...
          <div className="cart-content">
              ...
              {cartItems.map((item) => (
                <div key={item.productUid} className="cart-item-card">
                  <div className="item-img-container">
                    <img src={item.imageUrls?.[0] || item.imageUrl || "/no-image.png"}alt={item.productName || "Product"}
                      onError={(e) => e.target.src = "/no-image.png"}
                    />
                  </div>
                  <div className="item-details">
                    <h4 className="item-name">{item.productName || "Unknown Product"}</h4>
                    <p className="item-price-single">₹{(item.priceAtTime || item.price || 0).toFixed(2)} each</p>
                    <div className="item-controls">
                      <div className="quantity-wrapper">
                        <button className="qty-btn" onClick={() => handleQuantity(item.productUid, "dec")} disabled={item.quantity <= 1}>
                          <FaMinus size={10} />
                        </button>
                        <span className="qty-display">{item.quantity}</span>
                        <button className="qty-btn" onClick={() => handleQuantity(item.productUid, "inc")}>
                          <FaPlus size={10} />
                        </button>
                      </div>
                  ...
                  <div className="item-actions">
                    <button className={`action-btn wishlist-btn ${wishlist.includes(item.productUid) ? "active" : ""}`}
                      onClick={() => toggleWishlist(item.productUid)} title="Add to wishlist">
                      <FaHeart />
                    </button>
                    <button className="action-btn remove-btn" onClick={() => handleRemove(item.productUid)} title="Remove from cart">
                      <FaTrashAlt />
                    </button>
                  </div>
                </div>
            ...
            <div className="cart-summary-section">
              <div className="summary-card">
                <h3>Order Summary</h3>
                <div className="summary-row"><span>Subtotal ({cartItems.length} items)</span><span>₹{subtotal.toFixed(2)}</span></div>
